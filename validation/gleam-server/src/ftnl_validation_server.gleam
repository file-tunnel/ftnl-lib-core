import ftnl_validation.{type RequestMeta}
...
pub type InternalCommand {
  InternalCommand(operation_id: String, idempotency_key: Option(String), context: ServerRequestContext, payload: Dynamic)
}

pub fn trusted_actor_decoder() -> decode.Decoder(TrustedActor) {
  use user_id <- decode.field("userId", bounded_string(1, 128))
  use tenant_id <- decode.optional_field("tenantId", None, bounded_string(1, 128) |> decode.map(fn(value) { Some(value) }))
...
pub fn server_request_context_decoder() -> decode.Decoder(ServerRequestContext) {
  use public <- decode.field("public", ftnl_validation.request_meta_decoder())
  use actor <- decode.field("actor", trusted_actor_decoder())
  use source_ip <- decode.optional_field("sourceIp", None, ip_string() |> decode.map(fn(value) { Some(value) }))
...
pub fn internal_command_decoder() -> decode.Decoder(InternalCommand) {
  ...
  use idempotency_key <- decode.optional_field("idempotencyKey", None, bounded_string(1, 128) |> decode.map(fn(value) { Some(value) }))
  use context <- decode.field("context", server_request_context_decoder())
  use payload <- decode.field("payload", decode.dynamic)
  decode.success(InternalCommand(operation_id: operation_id, idempotency_key: idempotency_key, context: context, payload: payload))
}

pub fn decode_trusted_actor(value: Dynamic) { decode.run(value, trusted_actor_decoder()) }
pub fn decode_server_request_context(value: Dynamic) { decode.run(value, server_request_context_decoder()) }
pub fn decode_internal_command(value: Dynamic) { decode.run(value, internal_command_decoder()) }
