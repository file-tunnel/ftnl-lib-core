...
pub fn request_meta_decoder() -> decode.Decoder(RequestMeta) {
  ...
  use locale <- decode.optional_field("locale", None, bounded_string(2, 64) |> decode.map(fn(value) { Some(value) }))
...
pub fn page_query_decoder() -> decode.Decoder(PageQuery) {
  ...
  use cursor <- decode.optional_field("cursor", None, bounded_string(1, 512) |> decode.map(fn(value) { Some(value) }))
...
pub fn problem_details_decoder() -> decode.Decoder(ProblemDetails) {
  ...
  use detail <- decode.optional_field("detail", None, bounded_string(0, 4096) |> decode.map(fn(value) { Some(value) }))
  use request_id <- decode.field("requestId", bounded_string(1, 128))
...
pub fn decode_page_query(value: Dynamic) { decode.run(value, page_query_decoder()) }
pub fn decode_problem_details(value: Dynamic) { decode.run(value, problem_details_decoder()) }
